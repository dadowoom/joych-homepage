...
MIGRATION_0059="${APP_DIR}/drizzle/0059_bulletin_default_view_mode.sql"
if [[ -f "${MIGRATION_0059}" ]]; then
  echo "[deploy] database migration: bulletin default view mode"
  node --input-type=module <<'NODE'
import fs from "node:fs/promises";
import mysql from "mysql2/promise";
...
try {
  ...
  const [rows] = await connection.execute(
    "SELECT id FROM app_migrations WHERE id = ? LIMIT 1",
    ["0059_bulletin_default_view_mode"],
  );
  if (Array.isArray(rows) && rows.length > 0) {
    console.log("[deploy] migration 0059 already applied");
  } else {
    const sql = await fs.readFile("drizzle/0059_bulletin_default_view_mode.sql", "utf8");
    const statements = sql.split(/;\s*(?:\r?\n|$)/).map((statement) => statement.trim()).filter(Boolean);
    for (const statement of statements) {
      await connection.query(statement);
    }
