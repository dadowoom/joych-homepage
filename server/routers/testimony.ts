...
const postInputSchema = z.object({
  ...
  content: requiredText(50000, "간증 내용을 입력해주세요."),
...
export const testimonyRouter = router({
  posts: publicProcedure.query(() => getPublishedTestimonyPosts()),

  post: publicProcedure
    .input(z.object({ id: idSchema }))
    .query(({ input }) => getPublishedTestimonyPostById(input.id)),

  myPosts: memberProtectedProcedure.query(({ ctx }) => getTestimonyPostsByAuthor(ctx.memberId)),
  ...
  createPost: memberProtectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (!id) {
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "간증 저장에 실패했습니다." });
      }
      return { id };
    }),
  ...
  updatePost: memberProtectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (!post || post.status === "deleted") {
        throw new TRPCError({ code: "NOT_FOUND", message: "간증 글을 찾을 수 없습니다." });
      }
      if (post.authorMemberId !== ctx.memberId) {
        throw new TRPCError({ code: "FORBIDDEN", message: "본인이 작성한 간증만 수정할 수 있습니다." });
      }
      const images = normalizeImages(input.images);
  ...
  deletePost: memberProtectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (post.authorMemberId !== ctx.memberId) {
        throw new TRPCError({ code: "FORBIDDEN", message: "본인이 작성한 간증만 삭제할 수 있습니다." });
      }
  ...
  createComment: memberProtectedProcedure
    .input(z.object({
      postId: idSchema,
      content: requiredText(5000, "댓글 내용을 입력해주세요."),
    }))
    .mutation(async ({ input, ctx }) => {
      const post = await getTestimonyPostById(input.postId);
      if (!post || post.status !== "published") {
        throw new TRPCError({ code: "NOT_FOUND", message: "댓글을 작성할 수 없는 간증 글입니다." });
      }
      ...
      if (!id) {
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "댓글 저장에 실패했습니다." });
      }
      return { id };
    }),
  ...
  deleteComment: memberProtectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (comment.authorMemberId !== ctx.memberId) {
        throw new TRPCError({ code: "FORBIDDEN", message: "본인이 작성한 댓글만 삭제할 수 있습니다." });
      }
  ...
  updateComment: memberProtectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (comment.authorMemberId !== ctx.memberId) {
        throw new TRPCError({ code: "FORBIDDEN", message: "본인이 작성한 댓글만 수정할 수 있습니다." });
      }
      await updateTestimonyComment(input.id, { content: input.content, status: "published" });
      return { success: true };
    }),
