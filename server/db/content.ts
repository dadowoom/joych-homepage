...
async function getEventGalleryScopeKey() {
  ...
  const [items, subItems] = await Promise.all([
    db
      .select({
        href: menuItems.href,
        galleryScopeKey: menuItems.galleryScopeKey,
      })
      .from(menuItems)
    ...
    db
      .select({
        href: menuSubItems.href,
        galleryScopeKey: menuSubItems.galleryScopeKey,
      })
      .from(menuSubItems)
  ...
  return (
    candidates.find(row => row.href === EVENT_GALLERY_HREF)?.galleryScopeKey ??
...
export async function getVisibleGalleryAlbumDetail(
  ...
  return items.length > 0 ? { album, items } : null;
}
...
export async function getVisibleGalleryAlbums(galleryScopeKey: string) {
  ...
  const [albums, photos] = await Promise.all([
    ...
    db
      .select({ albumKey: galleryItems.albumKey })
      .from(galleryItems)
      .where(
        and(
          eq(galleryItems.galleryScopeKey, scope),
...
export async function updateGalleryItem(
  ...
  await db.transaction(async tx => {
    const [current] = await tx
      ...
      .from(galleryItems)
      .where(
        and(eq(galleryItems.id, id), galleryScopeCondition(galleryScopeKey))
      )
      .limit(1);
    if (!current) return;
...
/** 갤러리 앨범 공통 정보 수정 */
export async function updateGalleryAlbumItems(
  ids: number[],
  data: Partial<InsertGalleryItem>,
  galleryScopeKey?: string | null
) {
  ...
  await db.transaction(async tx => {
    const affectedItems = await tx
      ...
      .where(
        and(
          ...
          galleryScopeCondition(galleryScopeKey)
        )
      );
    await tx
      .update(galleryItems)
    ...
    for (const { scope, albumKey } of Array.from(albumPairs.values())) {
      ...
      if (data.albumTitle !== undefined)
        albumData.title = data.albumTitle || "최근 행사 사진";
      if (data.albumDescription !== undefined)
        albumData.description = data.albumDescription;
      if (data.albumSortOrder !== undefined)
        albumData.albumSortOrder = data.albumSortOrder;
...
export async function updateQuickMenu(
  id: number,
  data: Partial<typeof quickMenus.$inferInsert>
) {
  const db = await getDb();
  if (!db) return;
...
/** 퀵메뉴 순서 일괄 변경 */
export async function reorderQuickMenus(
  ...
  await Promise.all(
    items.map(item =>
      db
        ...
        .where(eq(quickMenus.id, item.id))
    )
  );
}
...
export async function reorderGalleryAlbums(
  items: {
      ...
      items.map(async item => {
        if (item.albumKey) {
          await tx
            ...
            .where(
              and(
                eq(galleryItems.albumKey, item.albumKey),
                galleryScopeCondition(galleryScopeKey)
              )
            );
        ...
        if (item.albumTitle) {
          await tx
            ...
            .where(
              and(
                isNull(galleryItems.albumKey),
                eq(galleryItems.albumTitle, item.albumTitle),
                galleryScopeCondition(galleryScopeKey)
              )
            );
          return;
  ...
  const rows = await db
    ...
    .where(
      and(
        ...
        not(
          like(siteSettings.settingKey, `${WORSHIP_SCHEDULE_SETTING_PREFIX}%`)
        )
      )
    );
...
export async function getStaticPageContentByHref(
  href: string
...
export function getTranslationSettingKey(
  locale: string,
  scope: string,
  resourceId: string
) {
...
export async function getStoredTranslation(
  ...
  const row = await getSiteSetting(
    getTranslationSettingKey(locale, scope, resourceId)
  );
  if (!row?.settingValue) return null;
  try {
    return {
...
export async function upsertStoredTranslation(
  locale: string,
  scope: string,
  resourceId: string,
  content: unknown
...
// ─── 퀵메뉴 추가/삭제 ────────────────────────────────────────────────────────
/** 퀵메뉴 새 항목 추가 */
export async function createQuickMenu(data: {
  icon: string;
  label: string;
  href?: string;
  sortOrder?: number;
...
export async function getAllGalleryAlbumDetail(
  galleryScopeKey: string,
  albumKey: string
) {
  const db = await getDb();
...
export async function deleteGalleryAlbumRecord(
  galleryScopeKey: string,
  albumKey: string
) {
  const db = await getDb();
...
export async function createGalleryItemsForAlbum(
  ...
  return db.transaction(async tx => {
    ...
    const existingSortOrders = await tx
...
export async function createGalleryItem(data: {
  ...
  await db.transaction(async tx => {
    ...
    if (!data.isHomeGallery && scope && albumKey && result?.id) {
      const [album] = await tx
        .select({ coverImageId: galleryAlbums.coverImageId })
        .from(galleryAlbums)
        .where(galleryAlbumCondition(scope, albumKey))
        .limit(1);
...
export async function deleteGalleryItem(
  ...
  await db.transaction(async tx => {
    ...
    const albumKey = item.albumKey?.trim();
    const [album] =
      scope && albumKey && !item.isHomeGallery
        ? await tx
            .select({ coverImageId: galleryAlbums.coverImageId })
            .from(galleryAlbums)
            .where(galleryAlbumCondition(scope, albumKey))
    ...
    for (const { scope, albumKey } of Array.from(albumPairs.values())) {
      ...
      if (remaining.length === 0) {
        ...
        await tx
          .update(galleryAlbums)
          .set({
            coverImageId: chooseGalleryCoverId(remaining, album?.coverImageId),
          })
          .where(galleryAlbumCondition(scope, albumKey));
      }
    }
