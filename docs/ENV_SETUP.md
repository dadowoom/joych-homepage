...
export PUBLIC_URL_BASE="https://newjoych.co.kr"
```
...
| `PUBLIC_URL_BASE` | OAuth 콜백 기준 주소(기존 PWA 호환 통로) | `https://newjoych.co.kr` |

> 대표 웹 주소는 `https://www.joych.org`입니다. 기존 Newjoych PWA의 소셜
> 로그인과 이미 등록된 Google/Kakao 콜백을 유지하기 위해 `PUBLIC_URL_BASE`만
> 호환 주소로 남깁니다. 이 값은 일반 페이지의 표시 주소를 결정하지 않습니다.
...
```
https://www.joych.org/api/member-oauth/google/callback
https://www.joych.org/api/member-oauth/kakao/callback
```
