...
운영 배포는 root 직접 접속보다 별도 배포 계정을 사용하는 것을 권장합니다.

- 예시 배포 계정: `joychdeploy`
- 앱 경로 `/var/www/joych-homepage`에 대한 쓰기 권한 부여
- PM2 재시작은 `/usr/local/bin/joych-pm2-restart joych-homepage`만 passwordless sudo 허용
- GitHub Secret `JOYCH_SSH_USER`에는 root가 아니라 배포 계정명을 등록

서버에 `/usr/local/bin/joych-pm2-restart`가 있으면 배포 스크립트는 해당 제한 스크립트를 통해 PM2를 재시작합니다. 해당 파일이 없는 개발 서버에서는 기존처럼 현재 사용자 권한의 `pm2 restart`를 사용합니다.
